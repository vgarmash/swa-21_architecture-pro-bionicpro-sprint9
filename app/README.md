...
docker-compose up -d
⚠️ Первый запуск может занять несколько минут —尤其 olap_db (ClickHouse) ожидает healthcheck.
...
ClickHouse healthcheck проверяет доступность через clickhouse-client (в контейнере). Если зависает — проверьте скрипты в ./olap-db/.
Готово! 🎉