...
def extract_sensors_data(**context):
    ...
    # Подключение к sensors-db (конфигурация через environment variables)
...
def extract_crm_data(**context):
    ...
    # Подключение к crm-db (конфигурация через environment variables)
    conn = psycopg2.connect(
...
def load_to_olap(**context):
    ...
    # Подключение к ClickHouse (конфигурация через environment variables)
    client = Client(
